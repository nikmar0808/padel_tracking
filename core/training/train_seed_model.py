...
from config_training import TRNG_DATASET_PATHS, SEED_EPOCHS, IMG_SIZE, BATCH_SIZE, DATALOADER_WORKERS

print("Starting seed training at timestamp:", datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
...
def train_model():
    ...
    # If you want to start with the model trained on batch 2 for a better starting point, 
    # but batch 0 is recommended for maximum performance gains and learning from scratch on the manual annotations
    # model = YOLO(r"runs\detect\train_batch2_ball\weights\best_batch2_ball.pt")
...
if __name__ == '__main__':
    train_model()
    print("Seed training complete! at timestamp:", datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
